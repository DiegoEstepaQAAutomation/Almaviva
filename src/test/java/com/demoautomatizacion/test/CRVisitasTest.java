...
@Epic("Consulta Regitro de Visitas")
@Feature("Consulta Regitro de Visitas Test")

public class CRVisitasTest extends BaseTest {
	...
	@Test(priority = 31, description = "Caso 33")
	@Severity(SeverityLevel.NORMAL)
	@Description("")
	@Story("Consulta Regitro de Visitas")
	@TmsLink("XRPRJ-1")
	...
	public void b66 () throws Exception {
		home.irPortal(getProperties().getProperty("url"));
		login.ingresarCredenciales1(getProperties().getProperty("usuario"), getProperties().getProperty("password"));
		CRVisitas.Visitas().RV().Modificar2().CodicionesBodega1().ScrollAbajo();
